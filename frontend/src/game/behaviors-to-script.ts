...
import { compileAllBlueprints, blueprintsProduceCode } from './blueprint-to-script';
 ...
 *  `ov` optionally overrides numeric params with a JS expression (Blueprint data
 *  wires — e.g. `set score = level * 10`); behaviors pass none.
 *  Exported so the Blueprint compiler can reuse the exact same action emission. */
export function emitAction(a: Action, me: string, other: string, dt: string, elements: DrawingElement[], ov?: Record<string, string>): string {
    switch (a.kind) {
        case 'moveDir': {
        ...
        case 'score':
            return `game.score(${ov?.delta ?? num(a.delta)});`;
        case 'setVar':
            return `_setVar(${q(a.name)}, ${ov?.value ?? num(a.value)});`;
        case 'changeVar':
            return `_setVar(${q(a.name)}, (_vars[${q(a.name)}] || 0) + ${ov?.delta ?? num(a.delta)});`;
        case 'showVar':
 ...
 * `blueprints` (optional) is the owner-keyed set of free-form exec-flow graphs
 * ('' = scene, tag = sprite); their compiled fragments merge into the same script
 * sections as the behaviors.
 */
export function generateGameScript(elements: DrawingElement[], sceneBehaviors: Behavior[] = [], gameVars: GameVar[] = [], blueprints?: Record<string, Blueprint> | null): string {
    const sprites = taggedSprites(elements);
    const bpf = compileAllBlueprints(blueprints, elements);
    if (sprites.length === 0 && sceneBehaviors.length === 0 && !blueprintsProduceCode(blueprints)) return '';
    ...
    for (const r of bpf.recv) recvLines.push(`_on(${q(r.message)}, () => {\n${r.body}\n});`);
    if (recvLines.length) { L.push('// —— messages ——'); L.push(...recvLines); L.push(''); }

    startLines.push(...bpf.start);
    ...
    for (const [btn, blocks] of Object.entries(bpf.press)) (pressByBtn[btn] ||= []).push(...blocks);
    ...
    for (const body of bpf.tap) tapBlocks.push(`{\n${body}\n}`);
    if (tapBlocks.length) {
    ...
    // ── Blueprint exec-flow: tick-context fragments (tick / keyHold / timer / varReaches) ──
    for (const line of bpf.tick) L.push(line);
    L.push('});');
