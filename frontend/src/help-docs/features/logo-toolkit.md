...
| Colour | The path inherits the text colour as a solid fill (no stroke). **Before** outlining, *Text Color* sets the colour of the letters (*Background* paints a box behind them); **after**, the letters are a shape, so change them with **Fill** (the Fill swatch, or *Background* in the property panel). The Text Color and Font rows are hidden on an outlined path, since there is no text left for them to style. Type a **Label** on the path and they come back, for that label. |
| Fonts | Works with the bundled families **and any font you add from a file** (*＋ Add font…*, .ttf / .otf / .woff). A **variable** font outlines only at its default weight (usually Regular). Other weights ask you to add that weight's static file, instead of silently giving you Regular letterforms. |
...
**Apply** commits it as ordinary grouped paths in your current drawing style, selected and ready to move or restyle, and it undoes in a *single* step no matter how many shapes it contains. Nothing is left behind if you Cancel. Leave *Arm symmetry after* ticked and Kaleidoscope symmetry is pointed at the new mandala's centre and spoke count, so you can carry straight on by hand. While it's on, its purple spokes stay on screen (they're guides, not objects, so the Layers panel can't select them) and a **Kaleidoscope ×** chip shows in the status bar: click it to turn symmetry off. Deleting the mandala turns the symmetry it armed off for you, unless you've changed the mode or moved the centre since. Dragging a selected mandala moves the whole design; it no longer starts a connector arrow from one of its pieces.
