...
- **Custom & Google fonts**: the **Font** dropdown (property panel and the Touch Type controls) is searchable — type to filter, <kbd>↑</kbd>/<kbd>↓</kbd> + <kbd>Enter</kbd> to apply, each row previewed in its own face — and **hovering a font (or arrowing to it) previews it on the selected text**, as in Illustrator. Move off the list or press <kbd>Esc</kbd> and the original font comes back; only a click (or <kbd>Enter</kbd>) applies it, as one undo step. It stays on screen no matter how many fonts you've added. From it choose **＋ Add font…** to upload a `.ttf/.otf/.woff/.woff2`, or **🔍 Google Fonts…** to search a curated list of popular Google Fonts. The picker docks to the right with the canvas still visible: each click — or <kbd>↑</kbd>/<kbd>↓</kbd> + <kbd>Enter</kbd> from the search box — applies the font to your selection live (marked *✓ applied*), so you can try fonts back-to-back and close with **Done**, <kbd>Esc</kbd>, or ✕ when happy. Added fonts load immediately, persist across reloads, and work like any built-in font.
- **Font Family and Font Style are two dropdowns**, as in Illustrator. Font files are named `Family-Weight` (`Montserrat-Light`, `Montserrat-SemiBoldItalic`), so adding a whole family used to fill the list with entries that looked like unrelated typefaces. They are now grouped: **Font** lists each family once, and **Style** beneath it lists that family's weights and italics (*Light, Regular, SemiBold, Bold Italic…*). Switching family keeps the style you were in wherever the new family has it — Montserrat SemiBold → a family whose heaviest is Bold lands on Bold rather than resetting to Regular. The **Style** row only appears when the family actually has more than one style.
- **Variable fonts** work from a single file. Google Fonts downloads are variable files named like `Roboto-VariableFont_wght.ttf`. Add that one file with **＋ Add font…** and it shows up as *Roboto*, with every weight it supports (*Thin, Light, Regular, Medium, SemiBold, Bold, Black…*) in the **Style** dropdown, drawn from the font's real weight axis rather than a faked bold. There's no need to add each static weight. Google ships italics as a second file (`Roboto-Italic-VariableFont_wght.ttf`); add it too and the italic weights join the same family. Yappy reads the weight range from the file (TTF, OTF, WOFF). For WOFF2 it trusts the `VariableFont_wght` / `[wght]` in the file name and assumes Thin–Black. Only the weight axis is exposed; width (`wdth`) and other axes are not. Fonts you added before this version are detected on the next reload. **Create Outlines** can only outline a variable font at its default weight (usually Regular); for other weights, add that static file. The **Bold** and **Italic** buttons still work and are a two-state view of the same thing: Bold lights up for anything SemiBold or heavier.
