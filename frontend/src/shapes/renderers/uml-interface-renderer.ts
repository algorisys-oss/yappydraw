...
export class UmlInterfaceRenderer extends ShapeRenderer {
    protected renderArchitectural(context: RenderContext, _cx: number, _cy: number): void {
        ...
        const layout = calculateUml2SectionLayout(context.renderer, el, 'methodsText');
    ...
    private renderTexts(context: RenderContext, el: DrawingElement, layout: Uml2SectionLayout) {
        ...
        // 1. Header: <<interface>> stereotype + name (centered)
        if (el.containerText) {
            ...
            renderer.fillText('<<interface>>', el.x + el.width / 2, stereoY);
            renderer.restore();
