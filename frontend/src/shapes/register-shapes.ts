...
export function registerShapes() {
    ...
    const umlInterfaceRenderer = new UmlInterfaceRenderer();
    shapeRegistry.register('umlInterface', umlInterfaceRenderer);

    const umlEnumRenderer = new UmlEnumRenderer();
    shapeRegistry.register('umlEnum', umlEnumRenderer);

    const umlGeneralRenderer = new UmlGeneralRenderer();
    const umlTypes = ['umlActor', 'umlUseCase', 'umlNote', 'umlPackage', 'umlComponent', 'umlLifeline', 'umlFragment'] as const;
