...
export const WHATS_NEW: WhatsNewEntry[] = [
    ...
    { version: '0.8.219', date: '2026-08-25', items: [
        ...
        'Use a picture as the surface. Give the object an image fill and Inflate shades that picture as the surface of the body instead of pasting it on flat — paint or drop in a skin, inflate it, and light it. Pair it with the ordinary Drop Shadow to sit the object on the ground.',
        'Patterned clouds fill properly. A cloud filled with dots, a grid, a checker, crosshatch or noise used to stop in a dead straight line partway up, with the outline carrying on above it. The cloud had been drawing slightly outside its own frame — which is also why its selection handles cut through it. Fixed at every size and shape.',
        'Layers can be reordered again. Dragging a layer by its grip showed a red no-entry sign and nothing happened. Note that the grip (⋮⋮) at the left of the row is what starts a reorder — dragging the row itself sideways is the swipe that reveals lock, duplicate and delete.',
        'In a design, artwork stops at the edge of the page. Draw or drag past the edge and the overhang is trimmed neatly at the paper, exactly as it already came out in your exported PNG or PDF — and it no longer spills onto the next page. Whatever you are dragging or have selected still shows in full, so nothing gets cut off while you are moving it.',
        'One object no longer appears on two pages. It never was two — pages share one canvas, so an object hanging over an edge was showing on both, which is why deleting it from one page seemed to delete it from the other. Each object now belongs to a single page, including one dropped in the gap between two, which previously belonged to neither and quietly went missing from exports.',
    ] },
    { version: '0.8.218', date: '2026-08-23', items: [
        'Convert to Path keeps your curves. Turning a shape like the Database into an editable path used to trace it as a many-sided polygon — the smooth barrel came back as a row of straight chords you then had to fix by hand. Curves now come through exactly, so a Database becomes six points you can actually work with.',
        'And it keeps the cap. The ellipse across the top of a Database is drawn separately from its body, so converting used to hand you a bare barrel with the rim missing. You now get both, as two editable paths — which makes a converted Database a real starting point for building your own cylinder shapes.',
        'A cylinder can taper. There is a new Taper slider under Dimensions: slide it one way and the cylinder narrows towards the top, for a column or a pillar; slide it the other way and it narrows towards the base, for a pot, a vase or a tumbler. It stays a live cylinder the whole time — the angle and the cap perspective are still yours to change.',
        'Rounded corners stop being squared off. Combining rounded shapes with the Shape Builder or the Pathfinder, cutting them with the Knife, or giving them 3D depth used to throw the rounding away and hand back sharp corners. They keep their curves now — which also means rounding a rectangle and adding a 3D Extrude with some Bevel finally gives you a block with blunt edges, like a dice.',
        '3D blocks can be converted to a path. Cubes and blocks previously refused: the option was simply not there, so there was no way to nudge one into a shape of your own. You now get the outline of the whole solid as an editable path — and the Node tool, Warp, Turntable, the Knife and the Pathfinder work on them too.',
        'The Pen can leave a path open with the mouse or pen. Hold Ctrl (Cmd on a Mac) and click anywhere on the canvas and the path finishes as an open curve, with the Pen still in your hand for the next one. Pressing Enter or Escape always did this, but neither is much use with a stylus.',
        'Undo now steps back one point while you are drawing a path. Pressing Ctrl+Z halfway through the Pen or the Polyline used to throw away everything you had placed. It removes just the last point now — press it again to keep going back. Once the path is finished, Ctrl+Z is the normal undo again.',
        'Hidden layers stay out of your exports. Anything on a layer with its eye switched off was still drawn into every PNG, JPG, SVG and PDF — and, worse, it stretched the size of the exported image, so a stray object parked off to one side could leave your drawing small in a corner of a mostly empty picture. Exports now match what you see on the canvas.',
    ] },
    ...
    { version: '0.8.205', date: '2026-08-20', items: [
        ...
        'Sloppiness now starts at 0. It used to start at 1 — on a drawing style that ignores it — so the first time you switched to Sketch the app had already decided how rough you wanted it. Now you start clean and turn it up.',
        'Your existing drawings are unchanged: every shape keeps the sloppiness it was drawn with. Only new shapes start at 0.',
    ] },
