...
- [0.8.241](0.8.241.md) — Every YSL text diagram imported as a deck of blank slides and deleted the drawing underneath it. Three faults stacked, and the middle one is the general lesson: when two formats share one import box, the permissive detector wins whatever the order. `isMarkdownSlideContent` claims any text with a `---` line, because Markdown breaks slides that way; every YSL source opens with `---` frontmatter. Put Markdown first and it takes the diagrams; put it second and it still takes everything the strict test failed to recognise — so the fallback has to be chosen by what a format *cannot* contain, not by what it commonly does. Below it, `isYSLScript` read a bare `end` as proof of a script, which is both a flowchart's last box and a sequence diagram's `loop … end`; above it, `clearCanvas: true`, inherited from a v0.25.6 fix aimed at a different caller, made pasting a diagram destructive. Import is parked behind Dev Mode while the rest is worked through
- [0.8.240](0.8.240.md) — GIF export existed on the infinite canvas the whole time; the Export dialog just never offered it there, and the only other control lived in a presentation toolbar that does not mount while editing. Three implementations, an API surface, help docs and an e2e suite asserting the downloaded bytes, all passing, all testing the mechanism rather than the way in. The gate was defensible in isolation — the offline exporter needs page bounds — but WebM and MP4 have the identical constraint and were handled the opposite way eight lines below, with a fallback instead of a hide, so the file already contained its own answer. A lone `Show` among siblings that all cope with the same case is usually an unfinished branch, not a decision. The other half of the fix is the half that would have been the next bug: wiring the dialog to the live capture without also giving it a Stop that exists while editing
- [0.8.239](0.8.239.md) — The guard that would have caught the previous release's bug before it was pushed. `publish-oss.sh` now refuses to publish a tree missing anything the build reads, and the list is derived rather than hand-written, because a hand-maintained list goes stale exactly the way the `.ossignore` comment did: the prerenderer declares its inputs as `path.join(REPO, …)`, so the guard greps for that and inherits every future input for free. Plus `--verify`, which builds the published tree before pushing. The learning is the uncomfortable one: `OSS_KEEP` already existed because the same thing happened to `scripts/`, with an accurate comment explaining it, and it happened again anyway. A lesson in a doc changes what the next person knows; only a check changes what they can do
- [0.8.238](0.8.238.md) — v0.8.236 and v0.8.237 were published and never deployed: the live site served v0.8.235 for a day while Hostinger reported failed builds with no logs. That is bug #344's signature for an OOM kill, and it was the wrong answer — a clean clone of the OSS mirror plus `npm ci` and `npm run build` reproduced it in three minutes with a plain ENOENT the host never surfaced. `.ossignore` had started stripping `articles/` the day before, under a comment saying it was "not consumed by the build", and `readArticles()` scans exactly that directory to generate `/learn/`. `git tag --contains` named the two broken releases and no others. Fixed at both ends: exclude the one internal article rather than its parent, and make a missing directory fail with a message naming the cause instead of skipping — a silent skip would have shipped a green deploy with a whole indexed section 404ing
- [0.8.237](0.8.237.md) — `/founders/` becomes a contribute page: one payment, any amount from ₹100, and two published thresholds instead of a cohort. The scarcity apparatus dies for the second and last time — v0.8.232 hid the "1,000 of 1,000 places remaining" bar behind a flag, and its replacement, seasons on published dates, was cut before shipping for the reason its own code admitted in a comment: the page is prerendered, so a window would appear to close only on the next deploy. That is the counter's flaw wearing a date, and the general form is that a static site can only publish facts about itself. Also D3 in the collaboration plan reversed — founding members do get the server's source, framed as access rather than exclusivity, which makes D2 (the server imports nothing from `frontend/src`, or §13 pushes that source to everyone who connects) the thing the benefit now rests on rather than a tidy licence rule
- [0.8.236](0.8.236.md) — Ten fixes and three improvements from a designer's first sustained week in the app, working the way a vector illustrator actually works: many layers, many colour experiments, the Pen, repeated exports. Four of the ten were one shape — a rule implemented twice and updated once — with the tell in every case being a comment asserting an invariant instead of code enforcing it. Exports walked `store.elements` in array order while the canvas drew by `layer.order`, identical for one layer and wrong for anything else
- [0.8.235](0.8.235.md) — The v0.8.234 deploy failed with no build logs at all, and the host's own analysis blamed four things after admitting it could not see the error. The empty log WAS the diagnosis: a process that fails writes one, a process that is SIGKILLed cannot, so "no logs" narrows the field to things that kill rather than fail. `vite build` was being OOM-killed at ~2.2 GB. Not caused by that release either — measured across clean clones the predecessor peaked at 1.88–2.14 GB against 2.17–2.24 GB, two ranges overlapping inside one version's own variance, so v0.8.233 had not passed by being under the limit but by drawing 1.88 GB that run. Capping V8's heap for the one step that needs it takes the peak to 1.65 GB, with the floor measured (1024 and 768 MB both still OOM) and the output proved byte-identical rather than assumed to be
- [0.8.234](0.8.234.md) — Dev Mode: a switch that hides features still being built, off by default, with the Arcade game builder first behind it. The interesting part is not the toggle but where the value lives — one named `isDevMode()` rather than a field read at each site, so `grep isDevMode` lists the app's whole hidden surface, and one place decides what "on" means. Only the entry points are hidden: unlike Teaching mode, which genuinely disables tools because hiding a button there would strand you in one, the game APIs and existing documents keep working. The line the tests actually protect is the `loadDocument` override — a `.yappy` saved by a developer carries `devMode: true` inside it, and without it, opening someone's file would unlock unfinished UI on your machine. Plus #343: the Arcade doc's first step named a menu path that had not existed for releases, invisible because nobody reads a help page for a feature they already know how to open
