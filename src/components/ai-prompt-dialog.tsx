...
import { X, Sparkles, Loader2, AlertTriangle, Check, Settings } from "lucide-solid";
...
const AIPromptDialog: Component<AIPromptDialogProps> = (props) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const res = await generateDiagram(text, { clearCanvas: clearCanvas() });
            setResult(res);
            if (res.success) {
    ...
    return (
        <Show when={props.isOpen}>
            <div class="ai-prompt-overlay" onClick={() => !isGenerating() && props.onClose()}>
